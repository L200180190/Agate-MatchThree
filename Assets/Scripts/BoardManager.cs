...
public class BoardManager : MonoBehaviour
{
    ...
    private void ProcessMatches(){
        ...
        // Hentikan lock jika tidak ada match yg ditemukan
    ...
    private Dictionary<TileController, int> GetAllDrop(){
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                {
                    // Proses untuk semua tile di atas destroyed tile
                    for (int i = y + 1; i < size.y; i++)
                    {
                        ...
                        // Jika tile sudah ada di drop list, naikkan drop range
                        if (droppingTiles.ContainsKey(tiles[x, i]))
                        {
                            droppingTiles[tiles[x, i]]++;
                        }

                        // Jika tidak ada di drop list, tambahkan dengan drop range one
                        else
                        {
    ...
    private IEnumerator RepositionTiles(System.Action onCompleted){
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                // Lewati jika sudah tepat posisinya
...
// Create function to generate tile, dengan tambahan u/ mengatur posisi agar berada di tengah layar dan dpt dicustom
    ...
    public List<TileController> GetAllMatches(){
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                // Pergi ke tile berikutnya jika tidak match
                ...
                {
                    // Tambahkan hanya 1 yang belum ditambahkan sebelumnya
